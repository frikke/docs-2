...
**Note:** this is the "per-architecture" repository for the `arm32v6` builds of [the `couchdb` official image](https://hub.docker.com/_/couchdb) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![arm32v6/couchdb build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/couchdb.svg?label=arm32v6/couchdb%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/couchdb/)
...
```console
$ docker run -d --name my-couchdb arm32v6/couchdb:tag
```
...
```console
$ docker run -p 5984:5984 -d arm32v6/couchdb
```
...
```bash
$ docker run --name some-%%REPO% -v /home/couchdb/data:/opt/couchdb/data -d arm32v6/couchdb:tag
```
...
```console
$ docker run -e COUCHDB_USER=admin -e COUCHDB_PASSWORD=password -d arm32v6/couchdb
```
...
```console
$ docker run --name my-couchdb -v /home/couchdb/etc:/opt/couchdb/etc/local.d -d arm32v6/couchdb
```
