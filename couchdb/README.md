...
**Note:** this is the "per-architecture" repository for the `i386` builds of [the `couchdb` official image](https://hub.docker.com/_/couchdb) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![i386/couchdb build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/i386/job/couchdb.svg?label=i386/couchdb%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/i386/job/couchdb/)
...
```console
$ docker run -d --name my-couchdb i386/couchdb:tag
```
...
```console
$ docker run -p 5984:5984 -d i386/couchdb
```
...
```bash
$ docker run --name some-%%REPO% -v /home/couchdb/data:/opt/couchdb/data -d i386/couchdb:tag
```
...
```console
$ docker run -e COUCHDB_USER=admin -e COUCHDB_PASSWORD=password -d i386/couchdb
```
...
```console
$ docker run --name my-couchdb -v /home/couchdb/etc:/opt/couchdb/etc/local.d -d i386/couchdb
```
