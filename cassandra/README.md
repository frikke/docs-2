...
[![amd64/cassandra build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/amd64/job/cassandra.svg?label=amd64/cassandra%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/amd64/job/cassandra/)
...
```console
$ docker run --name some-cassandra --network some-network -d amd64/cassandra:tag
```
...
```console
$ docker run --name some-cassandra2 -d --network some-network -e CASSANDRA_SEEDS=some-cassandra amd64/cassandra:tag
```
...
```console
$ docker run --name some-cassandra -d -e CASSANDRA_BROADCAST_ADDRESS=10.42.42.42 -p 7000:7000 amd64/cassandra:tag
```
...
```console
$ docker run --name some-cassandra -d -e CASSANDRA_BROADCAST_ADDRESS=10.43.43.43 -p 7000:7000 -e CASSANDRA_SEEDS=10.42.42.42 amd64/cassandra:tag
```
...
```console
$ docker run -it --network some-network --rm amd64/cassandra cqlsh some-cassandra
```
...
To use a different file name (for example, to avoid all image-provided configuration behavior), use `-Dcassandra.config=/path/to/cassandra.yaml` as an argument to the image (as in, `docker run ... amd64/cassandra -Dcassandra.config=/path/to/cassandra.yaml`).
...
2.	Start your `cassandra` container like this:
	...
	```console
	$ docker run --name some-cassandra -v /my/own/datadir:/var/lib/cassandra -d amd64/cassandra:tag
	```
