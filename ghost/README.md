...
```console
$ docker run -d --name some-ghost -e NODE_ENV=development amd64/ghost
```
...
```console
$ docker run -d --name some-ghost -e NODE_ENV=development -e url=http://localhost:3001 -p 3001:2368 amd64/ghost
```
...
```console
$ docker run -d --name some-ghost -e NODE_ENV=development -e url=http://some-ghost.example.com amd64/ghost
```
