...
```console
$ docker run -d --name some-ghost -e NODE_ENV=development arm32v7/ghost
```
...
```console
$ docker run -d --name some-ghost -e NODE_ENV=development -e url=http://localhost:3001 -p 3001:2368 arm32v7/ghost
```
...
```console
$ docker run -d --name some-ghost -e NODE_ENV=development -e url=http://some-ghost.example.com arm32v7/ghost
```
