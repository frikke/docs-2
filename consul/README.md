...
**Note:** this is the "per-architecture" repository for the `arm32v6` builds of [the `consul` official image](https://hub.docker.com/_/consul) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![arm32v6/consul build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/consul.svg?label=arm32v6/consul%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/consul/)
...
```console
$ docker run -d --name=dev-consul -e CONSUL_BIND_INTERFACE=eth0 arm32v6/consul
```
...
```console
$ docker run -d -e CONSUL_BIND_INTERFACE=eth0 arm32v6/consul agent -dev -join=172.17.0.2
... server 2 starts
$ docker run -d -e CONSUL_BIND_INTERFACE=eth0 arm32v6/consul agent -dev -join=172.17.0.2
... server 3 starts
```
...
```console
$ docker exec -t dev-consul arm32v6/consul members
...
```console
$  docker run -d --net=host -e 'CONSUL_LOCAL_CONFIG={"leave_on_terminate": true}' arm32v6/consul agent -bind=<external ip> -retry-join=<root agent ip>
...
```console
docker run -d --net=host arm32v6/consul agent -bind=<external ip> -client=<bridge ip> -retry-join=<root agent ip>
==> Starting Consul agent...
==> Starting Consul agent RPC...
==> Consul agent running!
         Node name: 'linode'
...
```console
$ docker run -d --net=host -e 'CONSUL_LOCAL_CONFIG={"skip_leave_on_interrupt": true}' arm32v6/consul agent -server -bind=<external ip> -retry-join=<root agent ip> -bootstrap-expect=<number of server agents>
```
...
```console
$ docker run -d --net=host -e 'CONSUL_ALLOW_PRIVILEGED_PORTS=' arm32v6/consul -dns-port=53 -recursor=8.8.8.8
```
...
```console
$ docker run -d --net=host -e 'CONSUL_ALLOW_PRIVILEGED_PORTS=' arm32v6/consul agent -dns-port=53 -recursor=8.8.8.8 -bind=<bridge ip>
```
