...
[![ppc64le/postgres build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/postgres.svg?label=ppc64le/postgres%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/postgres/)
...
```console
$ docker run -it --rm --network some-network ppc64le/postgres psql -h some-postgres -U postgres
psql (14.3)
Type "help" for help.
...
-	Use a custom config file. Create a config file and get it into the container. If you need a starting place for your config file you can use the sample provided by PostgreSQL which is available in the container at `/usr/share/postgresql/postgresql.conf.sample` (`/usr/local/share/postgresql/postgresql.conf.sample` in Alpine variants).
	...
	$ docker run -d --name some-postgres -v "$PWD/my-postgres.conf":/etc/postgresql/postgresql.conf -e POSTGRES_PASSWORD=mysecretpassword ppc64le/postgres -c 'config_file=/etc/postgresql/postgresql.conf'
	```
...
-	Set options directly on the run line. The entrypoint script is made so that any options passed to the docker command will be passed along to the `postgres` server daemon. From the [PostgreSQL docs](https://www.postgresql.org/docs/14/app-postgres.html#id-1.9.5.14.6.3) we see that any option available in a `.conf` file can be set via `-c`.
	...
	```console
	$ docker run -d --name some-postgres -e POSTGRES_PASSWORD=mysecretpassword ppc64le/postgres -c shared_buffers=256MB -c max_connections=200
	```
...
```console
$ docker run -d -e LANG=de_DE.utf8 -e POSTGRES_INITDB_ARGS="--locale-provider=icu --icu-locale=de-DE" -e POSTGRES_PASSWORD=mysecretpassword ppc64le/postgres:15-alpine 
```
...
```console
$ docker run -it --rm --user www-data -e POSTGRES_PASSWORD=mysecretpassword ppc64le/postgres
The files belonging to this database system will be owned by user "www-data".
...

$ docker run -it --rm --user 1000:1000 -e POSTGRES_PASSWORD=mysecretpassword ppc64le/postgres
...
2.	bind-mount `/etc/passwd` read-only from the host (if the UID you desire is a valid user on your host):
	...
	```console
	$ docker run -it --rm --user "$(id -u):$(id -g)" -v /etc/passwd:/etc/passwd:ro -e POSTGRES_PASSWORD=mysecretpassword ppc64le/postgres
...
3.	initialize the target directory separately from the final runtime (with a `chown` in between):
	...
	$ docker run -it --rm -v pgdata:/var/lib/postgresql/data -e POSTGRES_PASSWORD=mysecretpassword ppc64le/postgres
...
**Important note:** There are several ways to store data used by applications that run in Docker containers. We encourage users of the `ppc64le/postgres` images to familiarize themselves with the options available, including:
...
2.	Start your `ppc64le/postgres` container like this:

	```console
	$ docker run --name some-postgres -v /my/own/datadir:/var/lib/postgresql/data -e POSTGRES_PASSWORD=mysecretpassword -d ppc64le/postgres:tag
	```
...
## `ppc64le/postgres:<version>-alpine`
