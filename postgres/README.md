...
```console
$ docker run -d -e LANG=de_DE.utf8 -e POSTGRES_INITDB_ARGS="--locale-provider=icu --icu-locale=de-DE" -e POSTGRES_PASSWORD=mysecretpassword arm64v8/postgres:15-alpine 
```
...
The `arm64v8/postgres` images come in many flavors, each designed for a specific use case.
