...
```console
$ docker run --detach --name some-mariadb --env MARIADB_USER=example-user --env MARIADB_PASSWORD=my_cool_secret --env MARIADB_ROOT_PASSWORD=my-secret-pw  s390x/mariadb:latest
```
...
$ docker run --detach --network some-network --name some-mariadb --env MARIADB_USER=example-user --env MARIADB_PASSWORD=my_cool_secret --env MARIADB_ROOT_PASSWORD=my-secret-pw  s390x/mariadb:latest
```
...
The following command starts another `s390x/mariadb` container instance and runs the `mariadb` command line client against your original `s390x/mariadb` container, allowing you to execute SQL statements against your database instance:

```console
$ docker run -it --network some-network --rm s390x/mariadb mariadb -hsome-mariadb -uexample-user -p
```
...
```console
$ docker run -it --rm s390x/mariadb mariadb -h <server container IP> -u example-user -p
```
...
The `s390x/mariadb` image configuration contains the Ubuntu MariaDB variables with two custom changes for the container:
...
These disable the authentication of `user@hostname` users. To re-enable the `skip-name-resolve` use `disable-skip-name-resolve` as variable or arguement. When enabled, the `host-cache-size` should be sufficient for the number of containers connecting to the `s390x/mariadb`.

To view the resulting configuration of your `s390x/mariadb` container:

```console
$ docker run --name some-mariadb -v /my/custom:/etc/mysql/conf.d --rm s390x/mariadb:latest my_print_defaults --mysqld
```
...
```console
$ docker run --name some-mariadb -e MARIADB_ROOT_PASSWORD=my-secret-pw -d s390x/mariadb:latest --port 3808
```

If you would like to see a complete list of available options, just run:

```console
$ docker run -it --rm s390x/mariadb:latest --verbose --help
```
...
```console
$ docker run --name some-mysql -e MARIADB_ROOT_PASSWORD_FILE=/run/secrets/mysql-root -d s390x/mariadb:latest
```
...
2.	Start your `s390x/mariadb` container like this:
	...
	```console
	$ docker run --name some-mariadb -v /my/own/datadir:/var/lib/mysql -e MARIADB_ROOT_PASSWORD=my-secret-pw -d s390x/mariadb:latest
	```
...
$ docker run --name some-mariadb -v /my/own/datadir:/var/lib/mysql -v some-mariadb-socket:/var/run/mysqld -e MARIADB_MYSQL_LOCALHOST_USER=1 -e MARIADB_MYSQL_LOCALHOST_GRANTS="RELOAD, PROCESS, LOCK TABLES, BINLOG MONITOR" -e MARIADB_ROOT_PASSWORD=my-secret-pw -d s390x/mariadb:latest
```
...
$ docker run --rm some-mariadb-backup -v some-mariadb-backup:/backup s390x/mariadb:latest chown mysql: /backup
```

To perform the backup:

```console
$ docker run --user mysql -v some-mariadb-socket:/var/run/mysqld -v some-mariadb-backup:/backup -v /my/own/datadir:/var/lib/mysql --rm s390x/mariadb:latest mariabackup --backup --target-dir=/backup
```
...
```console
$ docker run --user mysql --rm -v some-mariadb-backup:/backup s390x/mariadb:latest mariabackup --prepare --target-dir=/backup
```

Now that the image is prepared, start the container with both the data and the backup volumes and restore the backup:

```console
$ docker run --user mysql --rm -v /my/new/datadir:/var/lib/mysql -v some-mariadb-backup:/backup s390x/mariadb:latest mariabackup --copy-back --target-dir=/backup
```

With `/my/new/datadir` containing the restored backup, start normally as this is an initialized data directory:

```console
$ docker run --name some-mariadb -v /my/new/datadir:/var/lib/mysql -d s390x/mariadb:latest
```
...
```console
$ docker run --rm -v /my/own/datadir:/var/lib/mysql -v /my/own/passwordreset.sql:/passwordreset.sql:z s390x/mariadb:latest --init-file=/passwordreset.sql
```
...
MariaDB has many plugins, most are not enabled by default, some are in the s390x/mariadb container, others need to be installed from additional packages.
...
To see which plugins are available in the s390x/mariadb:

```console
$ docker run --rm s390x/mariadb:latest ls -C /usr/lib/mysql/plugin
```
...
```console
$ docker run --name some-mariadb -e MARIADB_ROOT_PASSWORD=my-secret-pw --network=host -d s390x/mariadb:latest --plugin-load-add=simple_password_check
```
...
$ docker run --name some-mariadb -v /my/custom:/etc/mysql/conf.d -e MARIADB_ROOT_PASSWORD=my-secret-pw -d s390x/mariadb:latest
```
...
```console
$ docker run --rm s390x/mariadb:latest sh -c 'apt-get update -qq && apt-cache search mariadb-plugin'
```

### Creating a image with plugins from additional packages

A new image needs to be created when using additional packages. The s390x/mariadb image can be used as a base however:

In the following the [CONNECT Storage Engine](https://mariadb.com/kb/en/connect/) is installed:

```dockerfile
FROM s390x/mariadb:latest
RUN apt-get update && \
